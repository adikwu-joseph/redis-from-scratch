...
static bool read_u32(const uint8_t *&cur, const uint8_t *end, uint32_t &out){
    ...
    memcpy(&out, cur, 4);  // assume little endian
    cur += 4;
    return true;  
}

static bool read_str(const uint8_t *&cur, const uint8_t *end, size_t n, std::string &out) {
   if (cur + n > end) {  // should check n, not 4
    return false;
    }
...
//listen
    ...
    //a map of all connections, keyed by fd
    std::vector<Conn *> fd2conn;

    // the event loop
    std::vector<struct pollfd> poll_args; // for poll()
while (true) {
    ...
    for (Conn *conn : fd2conn) {
        ...
        //alwasy poll() for error events, to detect closed connections
